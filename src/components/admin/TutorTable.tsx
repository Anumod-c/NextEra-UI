...
const TutorTable: React.FC = () => {
  ...
  const handleBlockToggle = async (tutorId: string, currentStatus: boolean) => {
    try {
      await adminAxios.patch(adminEndpoints.changeTutorStatus(tutorId), {
  ...
  const handleVerificationToggle = async (
    ...
    try {
      await adminAxios.patch(adminEndpoints.changeTutorVerification(tutorId), {
        isVerified: !currentVerification,
      });
      setTutors((prevTutors) =>
        prevTutors.map((tutor) =>
          tutor._id === tutorId
            ? { ...tutor, isVerified: !currentVerification }
            : tutor
        )
      );
    } catch (error) {
              ...
              <tr key={tutor._id} className="border-b">
                ...
                <td className="p-2">
                  <button
                    ...
                    onClick={() =>
                      handleVerificationToggle(tutor._id, tutor.isVerified)
                    }
                  >
