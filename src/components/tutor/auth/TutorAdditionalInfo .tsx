...
const TutorAdditionalInfo = () => {
  ...
  const certificateInputRef = useRef<HTMLInputElement>(null); // Added ref for certificate
  const bucketName = import.meta.env.VITE_AWS_BUCKET_NAME;
  const region = import.meta.env.VITE_AWS_REGION;

  // Generate random image name using current timestamp
  ...
  const validationSchema = Yup.object().shape({
    ...
    cv: Yup.string().required('CV is required'), // Validation for CV
  });
  ...
  const handleFileUpload = async (
    ...
    try {
        ...
        if (fieldName === 'profilePicture') setPreviewImage(fileUrl);if (fieldName === 'certificate') {
            setCertificateFileName(file.name); // Set the file name for display
          }
      ...
      if (fieldName === 'cv') {
        setCvFileName(file.name); // Set CV file name for display
      }
  ...
  const handleSubmit = async (values: typeof initialValues, { setSubmitting }: FormikHelpers<typeof initialValues>) => {
    try {
      const response = await axios.post(tutorEndpoints.additionalInfo, values, {
        headers: {
            ...
            <Form className="space-y-6">
              ...
              <div className="flex flex-col items-center space-y-4">
                ...
                <input
                  ...
                  className="hidden"
                  accept="image/*"
                  onChange={(e) => handleFileUpload(e, setFieldValue, 'profilePicture')}
                />
                <ErrorMessage
                  name="profilePicture"
                  component="div"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div>
                  <label className="text-lg font-medium block mb-2">Expertise</label>
                    ...
                    <div key={index} className="flex items-center space-x-2 mb-2">
                      ...
                      {values.expertise.length > 1 && (
                        <button
                          type="button"
                          onClick={() => {
                            const newExpertise = values.expertise.filter((_, i) => i !== index);
                  ...
                  <button
                    ...
                    onClick={() => setFieldValue('expertise', [...values.expertise, ''])}
                  >
                    Add More
                  </button>
                </div>
                ...
                <div>
                  <label className="text-lg font-medium block mb-2">Qualifications</label>
                    ...
                    <div key={index} className="mb-4">
                      ...
                      <div className="flex items-center space-x-2 mt-2">
                        <button
                          ...
                          onClick={() => certificateInputRef.current?.click()} // Trigger file upload
                        >
                          <File className="h-5 w-5 text-blue-500 mr-2" /> Upload Certificate
                        </button>
                        {certificateFileName && (
                          <span className="text-sm text-gray-600">{certificateFileName}</span>
                        )}
                      </div>
                      <input
                        type="file"
                        ...
                        onChange={(e) => handleFileUpload(e, setFieldValue, `qualifications.${index}.certificate`)}
                      />
                    </div>
                  ))}
                  <button
                    type="button"
                    className="mt-2 px-3 py-2 rounded-lg bg-blue-500 text-white hover:bg-blue-600"
                    onClick={() => setFieldValue('qualifications', [...values.qualifications, { qualification: '', certificate: '' }])}
                  >
                    Add More
                  </button>
                </div>
              ...
              <div className='flex flex-col items-center'>
                <label className="text-lg  font-medium block mb-2">Upload CV (PDF only)</label>
                <div
